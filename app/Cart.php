    ...
    class Cart
{
    ...
    public function addProduct($product, $qty){
    	$products = ['qty' => 0, 'price' => $product->discount_price, 'product' => $product];
    	if($this->contents){
    	...
    	$products['qty']+=$qty;
    	$products['price'] = $product->discount_price * $products['qty'];
    	$this->contents[$product->slug] = $products;
    	$this->totalQty+=$qty;
    	$this->totalPrice += $product->discount_price;
    }
    ...
    public function updateProduct($product, $qty){
        ...
        $products['qty'] = $qty;
        $products['price'] = $product->discount_price * $qty;
        $this->totalPrice += $products['price'];
        $this->totalQty += $qty;
        $this->contents[$product->slug] = $products;
    }
