  ...
  <table class="table table-striped table-sm">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>₹{{$product->price}}</td>
        <td>{{dump($product->thumbnail)}}<img src="{{asset('storage/'.$product->thumbnail)}}" alt="{{$product->title}}" class="img-responsive" height="50"/></td>
        @if($product->trashed())
        ...
        <form id="delete-product-{{$product->id}}" action="{{ route('admin.product.destroy', $product->
        slug) }}" method="POST" style="display: none;">
